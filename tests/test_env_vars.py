...
def _env(**kwargs):
    env = {
        'CFTS_INPUT': 'microphone_1',
        'CFTS_INPUT_MICROPHONE_1_GAIN': '40',
        'CFTS_INPUT_MICROPHONE_1': 'some/calibration',
    }
    env.update(kwargs)
    return env
...
def _read(env, required_vars=('name', 'gain', 'calibration')):
    return read_env_vars('my_input', 'CFTS_INPUT', VAR_TEMPLATES,
...
class TestReadEnvVars:
    ...
    def test_missing_required_var_raises(self):
        env = _env()
        del env['CFTS_INPUT_MICROPHONE_1_GAIN']
        with pytest.raises(MissingEnvironmentVariables) as exc:
            _read(env)
        assert exc.value.missing == [('CFTS_INPUT_MICROPHONE_1_GAIN', 'gain')]
    ...
    def test_error_names_every_missing_variable(self):
        ...
        with pytest.raises(MissingEnvironmentVariables) as exc:
            _read({'CFTS_INPUT': 'microphone_1'})
        message = str(exc.value)
        assert 'CFTS_INPUT_MICROPHONE_1_GAIN' in message
        assert 'CFTS_INPUT_MICROPHONE_1' in message
    ...
    def test_missing_name_raises_when_required(self):
        ...
        assert exc.value.missing == [('CFTS_INPUT', 'name')]
        assert 'CFTS_INPUT' in str(exc.value)
    ...
    def test_defaults_to_os_environ(self, monkeypatch):
        ...
        values = read_env_vars('my_input', 'CFTS_INPUT', VAR_TEMPLATES,
    ...
    def test_full_environment_variable_name_in_template(self):
        ...
        # CFTS_TEST_STARSHIP but configured with CFTS_STARSHIP_<name>_*.
        env = {'CFTS_TEST_STARSHIP': 'ss_1', 'CFTS_STARSHIP_SS_1_GAIN': '40'}
        values = read_env_vars('starship', 'CFTS_TEST_STARSHIP',
                               {'gain': 'CFTS_STARSHIP_{name}_GAIN'},
                               ['name', 'gain'], env=env)
...
class TestReadNamedVars:
    ...
    def test_all_vars_set(self):
        env = _env()
        values = read_named_vars('my_input', 'microphone_1', 'CFTS_INPUT',
    ...
    def test_missing_required_var_raises(self):
        with pytest.raises(MissingEnvironmentVariables) as exc:
            read_named_vars('my_input', 'microphone_1', 'CFTS_INPUT',
