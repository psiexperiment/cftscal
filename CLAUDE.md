...
- `settings.py` — an `Atom`-based settings class that extends `CalibrationSettings`; members tagged `.tag(persist=True)` are auto-saved/loaded into the `[CFTSCAL_PLUGIN.<settings_filename>]` table of `config.toml`
...
- **`CalibrationSettings`** — base class for all plugin settings. `save_config()` / `load_config()` serialize `persist`-tagged Atom members into the `[CFTSCAL_PLUGIN.<settings_filename>]` table of the psi configuration file. `_run_cal(filename, experiment, env)` launches a calibration run via `subprocess` calling `psi <experiment> <filename>`.
...
These are ordinary psi settings — `CFTSCAL_ROOT`, `CFTSCAL_HW_MODE`, `CFTSCAL_DEVICE_NAME` and so on, living in `config.toml` — not a `workspace.json` of cftscal's own. See `WorkspaceSettings.SETTINGS` for the member-to-setting map and `cftscal/config_defaults.py` for the defaults. Per-plugin GUI state (`CalibrationSettings.save_config`) goes into a `[CFTSCAL_PLUGIN.<name>]` table each, rather than one JSON file each. A setting the environment is forcing cannot usefully be saved, so `overridden_settings()` reports those and the view disables the corresponding controls instead of accepting a write that changes nothing.

### Data Storage

Calibration root is `CFTSCAL_ROOT`, defaulting to `~/Documents/cftscal`. Resolve it with `get_config('CFTSCAL_ROOT')` at the point of use — there is deliberately no module-level constant. The previous one was captured at import and never reassigned, so changing the folder in the GUI left every later reader writing to the old tree until the process restarted. Each calibration run creates a timestamped directory `YYYYMMDD-HHMMSS_<metadata>/` containing CSV data and JSON metadata. Subdirectories per type: `microphone/`, `speaker/`, `starship/`, `input_amplifier/`, etc.

### Experiment Paradigms (`cftscal/paradigms/`)

Enaml-based experiment definitions consumed by `psi` (the psiexperiment runner). A calibration settings class calls `_run_cal(output_dir, 'cftscal.<paradigm_name>', env_vars)` to launch the experiment. Environment variables (e.g. `CFTSCAL_PISTONPHONE_LEVEL`, `CFTSCAL_TEST_STARSHIP`) pass hardware configuration to the paradigm.
...
- **Do not set `pg.setConfigOptions(useOpenGL=True)` in `cftscal/main.py`** — it was tried there once on the theory that it would stay isolated to cftscal's own process, since `psi` always runs calibrations as a separate subprocess (its own interpreter, its own pyqtgraph state). That reasoning doesn't hold for `PGCanvas` (`psi/data/plots_manifest.py` in psiexperiment): every cftscal plugin view imports and instantiates it directly, in-process, for its result plot, so the setting reaches it too. On at least one real system this reproducibly segfaulted (Windows access violation in `pg.GraphicsView.useOpenGL`) the instant any plugin workspace was selected — cftscal just vanished with no error message, no traceback. Confirmed by toggling the setting off and back on around the exact same `select_workspace` call path.
- **Filter design for level/PSD analysis** (`InputRecordingPlotManager._y_transform` in `cftscal/plugins/input_recording/view.enaml`): `filter_mode` is `'Unfiltered'` (true bypass, the default -- deliberately not called `'dBZ'`/"Z-weighting", since that name implies a standardized flat response over a defined range that this mode makes no claim to), `'dBA'` (standard IEC 61672-1 A-weighting, built from the analog pole/zero definition + bilinear transform — see `a_weighting_sos()`), or `'1/3 Octave'` (steep bandpass around `filter_fc`, built from `psiaudio.util.octave_band_freqs(fc, 1/3)`). Narrow/steep IIR filters should use `output='sos'` + `sosfiltfilt` rather than `'ba'` form + `filtfilt` — the polynomial ('ba') form gets numerically unstable at higher orders/narrower relative bandwidths. `_update_all()` (fired by filter_mode/filter_fc/filter_order changing) must call both `_update_plot()` *and* `_update_analysis()` — the PSD plot is drawn entirely inside the latter, so omitting it leaves the PSD stale after a filter change even though the time-domain curve updates fine.
- **`required_vars` on the `cftscal/paradigms/objects.enaml` manifests** (`Input`, `Output`, `Speaker`, `Microphone`, `Starship`, `InputAmplifier`): each one is configured from environment variables, and each declares a `required_vars` list naming the settings the experiment cannot run without. It defaults to *every* setting that manifest knows how to read, so a typo in an environment variable name fails loudly at `plugins_started` (`MissingEnvironmentVariables`, naming the exact variables expected) instead of silently running with whatever default the GUI shows — that silent fallback was the original behavior and it produced bad data. The flip side is that a paradigm which *creates* one of those calibrations has none to load, so it must override `required_vars` to drop the setting (see the mixins in `cftscal/paradigms/__init__.py`, whose overrides must match the `include_cal` arguments the corresponding plugin passes to `get_env_vars`). Settings left out of `required_vars` are still applied when the environment provides them. `record.enaml`'s `AllInputs` works the same way, per channel: its `required_vars` are `['channels', 'gain', 'calibration']`, where gain and calibration apply to *every* channel named in `CFTSCAL_INPUT_CHANNELS` (missing ones across all channels are collected into a single error). The shared reader is `cftscal/paradigms/env_vars.py` — `read_env_vars` when an environment variable names the hardware, `read_named_vars` + `check_required_vars` when the caller already knows the name. `tests/test_paradigm_objects.py` pins the expected `required_vars` for every cftscal paradigm.
- **Calibration status is reported, not inferred** (`cftscal/paradigms/calibration_status.py`): `Channel.calibration` defaults to `FlatCalibration.unity()` in psiexperiment, so a channel that never had a calibration loaded is indistinguishable after the fact from one that did — you cannot detect "missing" by inspecting the channel, and every other hardware setting is visible in its own widget while a calibration has none. Each object in `objects.enaml` therefore contributes a read-only `CalibrationParameter` context item (`{id}_calibration`; `{id}_input_calibration` for `Microphone`) that its init handler fills in with the calibration name and date, `NOT LOADED` when the hardware was selected but no calibration came with it, or the declared `not configured` when the object was never set up. `AllInputs` (`record.enaml`) contributes one per channel via a `Looper` over `CFTSCAL_INPUT_CHANNELS`. Being context items, these are saved with the data too. `CalibrationParameter` subclasses `EnumParameter` with a single choice purely for rendering: a context item's value is a Python *expression*, and psi draws a plain string `Parameter` as a field bound straight to that expression, quotes and all — the enum is drawn as a drop-down showing the choice *key*, so the text appears unquoted. Its `_default_choices` must quote directly rather than call `to_expression` (which reads `choices` and would recurse), and `dtype` is pinned to `'U'` so a name longer than whatever was in `choices` at creation is not truncated when saved.
